...
## 2026-09-07 ~01:05Z — WS-3.1b conflict-observation migration 0030 (planned block)

Step 13b (`docs/prompts/2026-09-05-48h-13b-migration-observations.md`), lane `ws3-conflict`,
branch `48h/ws3-conflict-20260905-mig-observations` from `origin/main` `a821695` (post-CP2b:
#63's 0028 and #64's 0029 both on `main`). D4 signed all 14 → C6 = (b) append-only.

1. Prove the worktree, base SHA, trimmed `.env.local` (only `NEON_API_KEY`); read COMMON,
   PLAN-WS-3 §3.1b, memo C3/C4/C6/C7/C13, the step-13 Handoff.
2. `conflict_validation_observations` in `src/db/schema.ts` verbatim from §3.1b under C6 = (b):
   no UNIQUE on `(conflict_id, reference_edition_id)`, partial UNIQUE on
   `(conflict_id, reference_edition_id, cron_run_id) WHERE cron_run_id IS NOT NULL`, the two
   read indexes, FKs to `benchmark_report_editions(id)` and `cron_runs(id)`, and the C3/C7/C13
   columns (`unit_attribution`, `gazetteer_version`, `unit_flags_version`).
3. Generate migration **0030** with `drizzle-kit generate --name conflict_observations`; verify
   the snapshot chain 0028 → 0029 → 0030 and that the SQL touches nothing but the new table.
4. `src/lib/conflicts/observation-store.ts`: `persistObservation` (assert-persistable first,
   refuse non-`retrospective`, refuse `fixture-oracle`, refuse a non-scored result, audit the
   result for prose before any write) + `latestObservationsFor` (latest row per edition, the
   headline derived at read time from the stored result).
5. `withCronRun(job, (counts, runId) => …)` — one optional second callback argument, additive.
6. `migrations.test.ts` 0030 pin (additive shapes, allowed column kinds, frozen tables, 9999
   last) + unit tests for the store and the cron-run argument.
7. Fork itest `src/integration/conflict-observations.itest.ts` on the `runMigrations` pattern:
   append-only replay, the partial unique, the FK refusal, read-time headline derivation over
   ≥3 rows, `unit_flags_version` stamped on every row.
8. Gates (typecheck/lint/unit + the fork itests), closing report
   `docs/reviews/WS-3-1-OBSERVATIONS-2026-09-06.md`, PR.

### Execution (same block)

- Worktree proven (`48h-ws3-conflict-20260905`, not the main checkout); base `origin/main`
  `a821695` == HEAD at start; `.env.local` trimmed (one `API_KEY` line, `NEON_API_KEY`);
  step branch `48h/ws3-conflict-20260905-mig-observations` cut from it. Attended session.
- Migration claimed is **0030** (`drizzle/0030_conflict_observations.sql`, journal `idx: 30`),
  generated by `drizzle-kit` with no hand-authored statement; chain 0028 → 0029 → 0030 verified
  by snapshot `prevId`/`id` equality on both links; `git diff drizzle/0028*` and `drizzle/0029*`
  both empty; a second `db:generate` reports "No schema changes"; 9999 still last.
- C6 = (b) built literally: no unique key on `(conflict_id, reference_edition_id)`, no
  `ON CONFLICT` clause, and no UPDATE/DELETE/MERGE anywhere in the store (source-scan pinned).
  The one unique key is the partial per-cron-invocation guard; the headline is derived at read
  time by `latestObservationsFor`'s `DISTINCT ON (reference_edition_id)`.
- Two strengthenings beyond the plan's wording, both recorded in the report: the row is a
  PROJECTION of the result (every result-determined column is read off it, never taken from the
  caller), and a ruling-1 prose audit runs before every write and again on every read — its
  alphabet MEASURED against the committed goldens, where `headlineLabel` and
  `window.cutoffAtRaw` are the only non-token strings the real scorer produces.
- `withCronRun` gains exactly one optional second callback argument (`runId`, null when the
  bookkeeping row could not be opened); nothing else in `cron-run.ts` moved.
- Eight mutations (prose column, widened unique key, mis-declared and missing column kind,
  deleted fixture-oracle refusal, deleted write-side audit, space-admitting alphabet, deleted
  read-side audit) each fail the suite; every mutated file restored and `cmp`-verified.
- Gates: typecheck clean · lint 0 errors (3 pre-existing warnings) · unit **3,914 → 3,954
  (263 → 264 files)**, both measured on this machine (baseline by detaching to `origin/main`)
  · `conflict-observations.itest.ts` 8/8 on fork `br-frosty-cherry-at7a1efl` · full integration
  **179 → 187 (26 → 27 files)** (baseline fork `br-empty-waterfall-atwuo7k6`, after fork
  `br-bitter-term-atq931fh`); every fork deleted by the runner. Spend $0.
- One AGENTS.md carve-out edit: `:116` `drizzle/  migrations 0000–0028` → `0000–0030` (the line
  was already stale at 0029 before this PR). The `src/lib/conflicts/` "pure" entry is left for
