...
| FIX (needs D-b) | **WS3-F03** | Per D-b — recommended (a): register an edition only when `units.length > 0`, count the body as indeterminate, keep the day `probe_failed`. Promote R2 with expectations inverted. Changes one PLAN §3.2a sentence: quote the before/after in the report for step 25. |
| FIX | **WS3-F04** | `observation-store.ts` `persistObservation`: `INSERT … SELECT … FROM benchmark_report_editions WHERE id = $2 AND edition_key = $5 AND series = $3 AND report_date = $4`; unit case + fork case for a mismatched id. **Step 19 runs in parallel in `ws3-conflict` and calls this function** — keep the signature unchanged; the operator merges 19 first at CP6 and you rebase. |
| FIX (needs D-c = (a)) | **WS3-F05** | `unitSignaturesFrom(html, gazetteerFor(series))`; stamp the gazetteer into `EDITION_UNITS_VERSION`; fix the vocabulary test to assert against the gazetteer actually used. Zero production rows exist, so no backfill. |
| FIX (needs D-e) | **WS3-F06** | Append the R3 variants to `iran-levant-v1` (append-only; no result has persisted under the version); word-mode-only apostrophe/hyphen fold in `match.ts` (substring mode untouched so the RU/UA proof stands); land the 54-probe script as a test. |
...
| per D-d | **WS3-F07** | If D-d names lane C: build `--backfill-from-isw-reports` per PLAN-WS-3 §3.2a with per-row typed refusals counted, never aborting, and 2025-06-12 → 06-24 excluded (#116); N3 reads "after the script exists"; $0, fork only. If D-d names step 19, record only. |
...
confirmations. Merge order for CP6 is the operator's: 19 first, then 23c, then 23r; any conflict
