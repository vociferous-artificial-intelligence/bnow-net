...
Marks adjusted 2026-09-10 after a second read of both registers: WS2-F01 and F23 into the drain bundle (gate pins, same files), WS2-F02 and F31 split docs → step 25 / code → DEFER, WS3-F10 to FIX beside F06. Shape chosen (plan §5.3): the five majors FIX; minors and notes that touch a spend path, a
...
| FIX (bundle: drain hardening + pins; one PR, or two — hardening then pins) | **WS2-F03, F24, F25, F26, F52, F54** + pins **WS2-F01, F23** | `src/lib/logs/drain.ts` / `route.ts`: F01 — in the `drain signature` describe, flip ONE hex char of a valid signature at index 0, 20 and 39 and assert `verifyDrainSignature` false (a first-4-bytes compare survives today); F23 — `route.test.ts` pins the three ordering properties (secret unset → 503 with `req.text`/`arrayBuffer` never called; verify before any parse; size cap after verification), same shape as WS3-F09 in lane C; NUL strip in `str()`/message + int4 clamp (F03); identifier-suffix redaction rule for `*_SECRET/_TOKEN/_API_KEY=` (F24); `posInt` floors before the `> 0` check, tests "0.5"/"0.99" (F25); `LOG_DRAIN_MAX_ROWS` clamped to 5,000 + test (F26); hex regex `{40}` + odd-length test (F52); HMAC over `req.arrayBuffer()` bytes, not the re-encoded text (F54). Invert the G5 describe in the same PR and run `runtime-logs.itest.ts` on a fork (name the branch in the report). |
...
| → step 25 (docs) | WS2-F08 (do-not-apply), F02 (docs half — `docs/designs/LOG-DRAIN.md` :228-229 / :490-500 / :543 and the review: the one-JOIN correlation argument is false for the ingest family; this is half of #62's fix-before-deploy verdict and must land before deploy), F09, F13 (docs half), F18, F19, F20 (docs half), F27, F28, F30, F31 (docs half — the report's `auto_scorecard_missing` / `route_policy` sentence is false as written), F33, and notes F11, F34, F35, F38, F40, F44, F45, F62, F64, F69, F70, F71 | Not this step's. Confirm they are on the register's "Stale standing text" list or add them to the report's step-25 list. |
...
| FIX (with F06, same PR) | **WS3-F10** | Tighten the `iran-levant-v1` invariant regex so a variant cannot carry a trailing space, and assert every `variantSource` ends anchored or stemmed — D-e appends variants this window, so "no such variant exists today" no longer defers it. |
...
| DEFER → #119 | WS3-F11, N01, N02, N05, N06, N07, N08, N09, N10 | Same bundle entry as lane R (append after rebase). N09: confirm step 17's ruling-5 check covers the 0029 pin before deferring. |
