...
| G materials/datadark/provenance | DONE | G1: migration 0019 (trade_flows.partner_name nullable, chains 0018); comtrade parser carries partnerDesc + includeDesc=true requested (verifiable only from Vercel egress — parser tolerant either way); pulls persist partner_name (COALESCE-keep); read path = stored → full 193-code M49 map (src/lib/trade/partners.ts) → explicit 'Partner code N' (never #N); prod-observed codes all name-mapped + tested; reporter/partner swap pinned. NO prod backfill needed (read-path map covers legacy rows deterministically). G2: extractPeriod = matchAll + latest comparable date (CBR form-default scenario fixture-pinned); staleness = period AGE vs poll instant (first-seen 2013 → stale immediately); anomaly guard keeps credible newer stored period + history entry without bumping last_changed_at; causal sentence → 'suppression can be an analytical signal…corroborated'. Prod cbr-key-rate row self-corrects on first post-deploy cron (no manual repair). G3: UN Comtrade portal+docs links (safe rel), reporter/flow/partner/HS/period/fetched-at exposed on both pages, S&P/CEPR/KSE names removed (no precise citation), mirror warning kept, uncited chokepoint percentages softened. CBR live check attempted — unreachable from this box; post-deploy cron verifies. |
